...
const SeatBooking = () => {
  ...
  return (
    <div className="h-screen w-screen bg-gradient-to-br from-gray-100 to-gray-300 flex flex-col overflow-hidden">
      ...
      {/* Main Content */}
      <div className="flex flex-row flex-1 max-w-7xl w-full mx-auto gap-4 p-4">
        {/* Seat Map */}
        <div className="w-2/3 h-[calc(100vh-100px)] p-6 bg-white rounded-xl shadow-xl flex flex-col">
          {loading ? (
        ...
        {/* Booking Form */}
        <div className="w-1/3 h-[calc(100vh-100px)] p-6 flex flex-col justify-center items-center bg-white rounded-xl shadow-xl">
          ...
          <button
            ...
            className="w-full max-w-sm bg-blue-600 text-white p-4 rounded-lg shadow-md hover:bg-blue-700 disabled:bg-gray-400 transition-all duration-300 transform hover:scale-105 text-lg"
          >
            Book Now
          </button>
          <button
            onClick={handleReset}
            className="w-full max-w-sm mt-4 bg-gray-600 text-white p-4 rounded-lg shadow-md hover:bg-gray-700 transition-all duration-300 transform hover:scale-105 text-lg"
          >
            Reset Booking
          </button>
        </div>
...
const SeatMap = ({ seats }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="grid grid-cols-7 gap-3 flex-1 overflow-auto">
        {seats.map((row, rowIndex) =>
          row.map((seat, colIndex) => (
            <div
              key={`${rowIndex}-${colIndex}`}
              className={`w-16 h-16 rounded-lg flex items-center justify-center text-white font-semibold shadow-md transition-all duration-300 transform hover:scale-110 ${
